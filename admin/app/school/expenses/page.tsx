    ...
    <section className="space-y-5 py-4">
      <Card className="border border-violet-100/70 bg-white/90 dark:border-violet-300/20">
        <CardHeader className="flex flex-wrap items-end justify-between gap-3 px-6 pt-6">
          ...
          <Button className="bg-violet-600 text-white" radius="full">Log Expense</Button>
        </CardHeader>
        <CardBody className="grid gap-3 px-6 pb-6">
          {expenses.map((expense) => (
            <div key={`${expense.item}-${expense.department}`} className="rounded-2xl border border-violet-100/70 bg-white p-4 dark:border-violet-300/20 dark:bg-slate-900/40">
