    ...
    <section className="space-y-5 py-4">
      <Card className="border border-violet-100/70 bg-white/90 dark:border-violet-300/20">
        <CardHeader className="flex flex-wrap items-end justify-between gap-3 px-6 pt-6">
          ...
          <Button as={Link} href="/school/parent/add" className="bg-violet-600 text-white" radius="full">Add Parent</Button>
        </CardHeader>
        <CardBody className="grid gap-3 px-6 pb-6">
          {parentRecords.map((parent) => (
            <div key={parent.name} className="rounded-2xl border border-violet-100/70 bg-white p-4 dark:border-violet-300/20 dark:bg-slate-900/40">
              <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                ...
                <div className="flex gap-2">
                  <Button as={Link} href={`/school/parent/${parent.name?.replaceAll(" ", "-")?.toLowerCase()}`} className="border-violet-600 text-violet-700" variant="bordered" radius="full">
                    View
                  </Button>
                  <Button className="bg-violet-600 text-white" radius="full">
                    Message
                  </Button>
                </div>
              </div>
