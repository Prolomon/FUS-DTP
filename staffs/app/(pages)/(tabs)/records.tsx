...
import { CheckInRecord, getCheckInRecords } from '@/lib/services/staff';
...
export default function RecordsScreen() {
  ...
  const fetchRecords = useCallback(async () => {
    ...
    if (!auth) return;
    try {
      const data = await getCheckInRecords(auth.schoolId as string, token as string);
      setRecords(Array.isArray(data) ? data?.filter(a => new Date(a.date).toISOString().split('T')[0] === new Date().toISOString().split('T')[0]) : data?.checkIns?.filter((a: any) => new Date(a.date).toISOString().split('T')[0] === new Date().toISOString().split('T')[0]) || []);
    } catch (error: any) {
