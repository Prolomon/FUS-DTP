...
export default function ResultScreen() {
	...
	const params = useLocalSearchParams<{ studentid?: string; name?: string; className?: string }>();
	const studentId = params.studentid || '';

	const foundReport = tempReports.find((report) => report.studentId === studentId) || null;
		...
		<SafeAreaView style={styles.container}>
			...
			<View style={styles.topBar}>
				<TouchableOpacity style={styles.backButton} activeOpacity={0.85} onPress={() => router.back()}>
					<ChevronLeft size={16} color="#1a2f44" strokeWidth={2.6} />
			...
			<ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
				{!foundReport && (
					<View style={styles.emptyCard}>
						<Text style={styles.emptyText}>No result found for this student.</Text>
					</View>
				)}

				{foundReport && (
					<View style={styles.reportCard}>
						<Text style={styles.searchTitle}>Student Result</Text>
						<View style={styles.studentHeaderRow}>
							<Image source={{ uri: foundReport.avatarUrl }} style={styles.avatar} />
							<View style={styles.studentInfoWrap}>
								<Text style={styles.studentName}>{params.name || foundReport.name}</Text>
								<Text style={styles.studentMeta}>Class: {params.className || foundReport.className}</Text>
